...
def modbus_command_generator(slave_id, function_code, register_address, value_array, byteorder, datatype):
    ...
    if not (datatype[0] in valid_data_types and datatype[1:3].isdigit() and (datatype[-1] == 'I' or datatype[-1] == 'F')):
    ...
    for value in value_array:
        ...
        if datatype[3]=='I':
            ...
            if [datatype[1],datatype[2]] == ['1','6']:
                if datatype[0] == 'S':
                    value_bytes = struct.pack(pack_sign+'h', value)  # Signed 16-bit integer
                elif datatype[0] == 'U':
                    value_bytes = struct.pack(pack_sign+'H', value)  # Unsigned 16-bit integer
                else:
                    raise ValueError("Invalid datatype")
            elif [datatype[1],datatype[2]] == ['3','2']:
                if datatype[0] == 'S':
                    value_bytes = struct.pack(pack_sign+'i', value)  # Signed 16-bit integer
                elif datatype[0] == 'U':
                    value_bytes = struct.pack(pack_sign+'I', value)  # Unsigned 16-bit integer
                else:
            ...
            if [datatype[1], datatype[2]] == ['3', '2']:
                value_bytes = struct.pack(pack_sign+'f', value)  # Float32
            else:
                raise ValueError("Datatype and bytesize mismatch")
        else:
            raise ValueError("Invalid datatype")
